...
function Connection(manager, options, req, socket, upgradeHead) {
    var _firstFrame, connection = this, send_to_parser = false;
	...
	socket.on('data', function(data) {
		...
		handshakes.draft76(connection, data);
	    }
	});
	...
	case 'draft76':
		...
		// deals with sending through proxies
		handshakes.draft76(connection, upgradeHead);
...
var handshakes = {
	...
	var location = websocket_location(connection), res;
	if('flushresponse' in connection){
	    connection.state(3);
		...
		if (connection._options.subprotocol &&
            ...
            res += '\r\n\r\n';
	    delete connection['flushresponse'];
	}
